var YiiChat = function(options){
	...
	var add = function(post){
		...
		else if(options.identity != post.post_identity)
			{ p.addClass('yiichat-post-other'); flag=true; }
		if(flag == true){
			if(options.myOwnPostCssStyle != null){
				p.removeClass('yiichat-post-myown');
				p.removeClass('yiichat-post-other');
		...
		p.find('.track').html("<div class='owner'>"+post.owner+"<span class='small'> , "+post.created+"</span></div>");
	...
	you.html("<textarea class='form-control' rows='5' placeholder='minimal 2 karakter, maksimal 140 karakter'></textarea><div class='exceded'></div><br/>");	
	you.append("<button class='btn btn-success'>"+options.sendButtonText+"</button>");
	posts.html("");
	...
	msg.keyup(function(e){
		var text = jQuery.trim(msg.val());
		if(text.length > options.maxPostLen){
			msg.css({ color: '#D24D57' });
			msg.parent().find(".exceded").text("Teks berlebihan. Maksimal 140 karakter.");
		}else{
			msg.css({ color: 'black' });
