 ...
 * @license FREE BSD
 */require_once(dirname(__FILE__).'/IYiiChat.php');
abstract class YiiChatDbHandlerBase extends CComponent implements IYiiChat {
	...
	public function yiichat_post($chat_id, $identity, $message, $data){
		...
		if($message_filtered != ""){
			$obj = array(
				...
				"created"=>date("Y-m-d H:i:s"),//time("Y-m-d H:i:s"),
			...
			$obj['date']=$this->getDateFormatted($obj['created']);
			return $obj;
		}
		else
			return array();
	...
	public function yiichat_list_posts($chat_id, $identity, $last_id, $data){
		...
		if($last_id == -1){
			...
			->queryAll();
			foreach($rows as $k=>$v)
				$rows[$k]['date']=$this->getDateFormatted($v['created']);
			return $rows;
		}
		else{
			...
			foreach($ar as $k=>$v)
				$ar[$k]['date']=$this->getDateFormatted($v['created']);
			return $ar;
		}
	}
